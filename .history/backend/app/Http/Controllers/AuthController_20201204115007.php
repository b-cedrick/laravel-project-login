...
class AuthController extends Controller
{
    ...
    public function login(Request $request) {
        ...
        if(!$token = auth()->attempt($validator->validated())) {
            ...
            $nb_login_attempts = 3;
            $client_ip = $this->_getClientIp();
            $message = 'Accès non autorisé';
            if($res[0] < 3){
                $nb_login_attempts = $res[0]+1;
            }else {
                //ecrire dans un fichier log
                Log::info('L\'utilisateur utilisant l\'email : '.$request->email.' a tenté de se connécté 3 fois avec un movais couple d\'identifiant / mots de passe');
                $nb_login_attempts = 0;
                $message = 'Vous avez atteint le nombre maximal de tentative, veuillez réssayer dans 1 minutes';
            }
            ...
            return response()->json($message.$client_ip, 401);
        }

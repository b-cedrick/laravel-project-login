    ...
    public function login(Request $request) {
        ...
        if(!$token = auth()->attempt($validator->validated())) {
            ...
            if($res[0]->nb_login_attempts < 3){
                ...
                $new_nb_login_attempts_value = $nb_login_attempts;
            }
            
            $affected = DB::table('users')
              ->where('email', $request->email)
              ->update(['nb_login_attempts' => $nb_login_attempts,'last_login_attemps'=>$                $new_nb_login_attempts_value = $nb_login_attempts;
              , 'ip_client'=>$client_ip]);

            // return response()->json(['error' => 'Unauthorized'], 401);
        $last_login_attemps = $res[0]->last_login_attemps;
            return response()->json($interval, 401);
        }

        return $this->createNewToken($token);
        }
        
        $temps_restant = 60 - $interval;

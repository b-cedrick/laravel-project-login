...
class AuthController extends Controller
{
    ...
    public function login(Request $request) {
        ...
        $interval_in_seconde = $last_login_attemps->diffInSecondsdiff($now);
        ...
        if($temps_restant <= 0 && $res[0]->nb_login_attempts < 3){
            ...
            $affected = DB::table('users')
            ->where('email', $request->email)
            ->update(['nb_login_attempts' => 0, 'ip_client'=>$client_ip]);               
            return $this->createNewToken($token);
        }        
        
        return response()->json('Il vous reste '.$temps_restant.' secondes avant de pouvoir vous reconnecter'.$last_login_attemps->diff($now)->format('%H:%I:%S'), 401);
    }

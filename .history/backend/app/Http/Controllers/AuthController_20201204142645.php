    ...
    public function login(Request $request) {
        ...
        if($temps_restant <= 0){
            ...
            if(!$token = auth()->attempt($validator->validated())) {
                ...
                if($res[0]->nb_login_attempts < $nb_login_attempts ){
                    
                    $nb_login_attempts = $res[0]->nb_login_attempts+1;
                    $message = 'Accès non autorisé : après '.($nb_login_attempts - $nb_login_attempts).' utres tentatives échoué votre compte sera bloqué temporairement!';
